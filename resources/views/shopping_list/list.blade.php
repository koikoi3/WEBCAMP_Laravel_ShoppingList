...
@foreach ($list as $shopping_list)
        <tr>
            ...
            <td><form style="margin: 0px;" action="{{ route('complete', ['shopping_list_id' => $shopping_list->id]) }}" method="post"> @csrf <button onclick='return confirm("この「買うもの」を「完了」にします。よろしいですか?");' >完了</button></form></td>
            <td width="20px"></td>
            <td><form style="margin: 0px;" action="{{ route('delete', ['shopping_list_id' => $shopping_list->id]) }}" method="post"> @csrf <button onclick='return confirm("この「買うもの」を「削除」します。よろしいですか?");' >削除</button></form></td>
@endforeach
        </table>
        <!--ページネーション-->
        {{--{{ $list->links() }} --}}
